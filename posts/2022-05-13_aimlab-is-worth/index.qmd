...
#| fig-height: 12
#| fig-dpi: 300
val_task <- readr::read_csv("data/val-task.csv", col_types = "fdd") %>%
  dplyr::filter(
    dplyr::between(dplyr::row_number(), 1, 
                   max(which(!is.na(time_to_complete)))))

val_task %>%
  ggplot2::ggplot() +
  ggplot2::aes(x = day, y = time_to_complete, group = 1) +
  ggplot2::stat_summary(fun = mean, geom = "point",
                        color = "#A034F0", size = 9, alpha = 0.6) +
  ggplot2::stat_summary(fun = mean, geom = "line",
                        color = "#A034F0", size = 5) +
  ggplot2::scale_y_continuous(
    breaks = scales::pretty_breaks(8),
    sec.axis = ggplot2::sec_axis(~100/., name = "Kills per Second",
                                 breaks = scales::pretty_breaks(8))) +
  ggplot2::labs(x = "Day", y = "Time to Complete (s)",
                caption = "Data: Aimlab | Visualization by @damarals_") +
  danlib::dan_theme(base_size = 60, axis_title_size = 55,
                    axis_text_x_size = 54, caption_plot_size = 35)
```
